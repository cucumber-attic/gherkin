    ...
    for examples in scenario_outline['examples']:
        ...
        for values in examples['tableBody']:
            ...
            for scenario_outline_step in scenario_outline['steps']:
                ...
                arguments = _create_pickle_arguments(scenario_outline_step.get('argument'), variable_cells, value_cells, path)
                _pickle_step = {
                    'text': step_text,
                    'arguments': arguments,
                    'locations': [
...
def _get_background_steps(feature, path):
    if 'background' in feature:
        return [_pickle_step(step, path) for step in feature['background']['steps']]
    return []

def _pickle_step(step, path):
    return {
        'text':      step['text'],
        'arguments': _create_pickle_arguments(step.get('argument'), [], [], path),
