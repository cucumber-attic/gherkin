...
### Changed
* (All) The AST's `DocString` `contentType` property is now undefined (rather than
  an empty string) when the Gherkin doc does not specify a type after three backticks
  (by [aslakhellesoy])
